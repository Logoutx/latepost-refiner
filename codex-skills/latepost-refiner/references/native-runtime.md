...
Default profile:
- `scout`, ending `check`: `gpt-5.4-mini`, low reasoning.
...
Spawn one native Codex subagent per refine prompt, using the prompt entry's high-quality model settings. Each subagent writes directly to its assigned
...
Spawn native subagents for `logic`, `summary`, and `timeline` prompts as requested, using each prompt entry's `model` and `reasoning_effort`. Use built-in browsing for timeline public-source checks. Save compact reports and paths in `result.json`.
...
or one-pass glossary to the audit, audits logic-order drafts against refined transcripts, and writes
