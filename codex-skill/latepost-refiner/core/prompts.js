...
export function summaryDeliverableName(topic) {
...
export function timelineDeliverableName(topic) {
  return `${safeName(topic, 40)}时间线.md`
}

export function summaryPrompt(a, refined, sectionMapPath) {
...
export function timelinePrompt(a, glossary, refined, sectionMapPath) {
  const list = refined.map((r) => '- ' + (r.outPath || r.path)).join('\n')
  const mapNote = sectionMapPath
...
export function logicPlanPrompt(f, a, sectionMapPath) {
...
export function logicWritePrompt(f, a, missing, planPath) {
  const outName = safeName(f.title)
