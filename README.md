...
> 完全不写代码、只想把工具跑起来？看 [给非技术同事的安装指南](docs/install-for-non-coders.md)。
...
- 打包 claude.ai 上传用的 zip：`bash claude-code-skill/build-zips.sh`（产出 `latepost-refiner.zip`；技能每次运行都会问你输出目录，不再有个人版/分享版之分）。
...
node universal/cli.js \
  --files "2025-02-21_王某 某事业部，渠道运营.txt" \
...
DEEPSEEK_API_KEY=sk-... node universal/cli.js --provider deepseek \
  --files "x.txt" --topic "示例公司" --scope refine --out ./out
```
