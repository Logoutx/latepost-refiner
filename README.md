...
```bash
npm install                              # 需 Node.js 20+，装 @anthropic-ai/sdk / openai / p-limit
cp .env.example .env && 编辑填入 ANTHROPIC_API_KEY   # CLI / 网页版会自动读取 .env；也可 export
...
| `glm` | `ZHIPUAI_API_KEY` / `ZAI_API_KEY` | `open.bigmodel.cn/api/paas/v4`（国际站 `--base-url https://api.z.ai/api/paas/v4`） | provider 原生搜索 |
| `kimi` | `MOONSHOT_API_KEY` | `api.moonshot.ai/v1`（国内 `--base-url https://api.moonshot.cn/v1`） | provider 原生搜索 |
...
- DeepSeek / OpenAI 无内置联网搜索：设 `TAVILY_API_KEY` 才能 verify/timeline 联网；不设则这两步降级（实体进 unresolved）。Anthropic / GLM / Kimi 使用 provider 原生搜索。**refine 精校不联网、完全不受影响**。
