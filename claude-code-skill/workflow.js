...
const READ_PAGE = 600          // 行数上限：密集中文 ~600 行 ≈ 18–22K tok，留足余量
const READ_BYTES_PER_PAGE = 45000  // 密度护栏：~45KB/页 ≈ 18K tok（中文 UTF-8 ≈ 2.5 B/tok）；只用于下调页大小防超限截断，绝不上调（页过大若超限会静默丢内容，省一次往返不值得）
function readPlan(f) {
  const n = f.lines || 0
...
// ---------- 主流程 ----------
// 防御：args 可能以 JSON 字符串到达（取决于调用端序列化），统一解析为对象 A
...
if (A.files.length === 1 && (A.files[0].lines || 9999) < 400) {
  ...
  const f = A.files[0]
  phase('Refine')
  log(`单份短文件（${f.lines} 行）：一遍过精校，不建独立校对表`)
  const rep = await agent(singlePassPrompt(f, A), { label: `refine:${f.label}`, model: M.refine, schema: REFINE_REPORT_SCHEMA })
  if (rep) {
    const chk = await agent(checkPrompt(f, null), { label: `check:${f.label}`, model: 'haiku', schema: CHECK_SCHEMA })
  ...
  phase('Scout')
  let findings = await parallel(A.files.map((f) => () =>
    agent(scoutPrompt(f, A), { label: `scout:${f.label}`, model: M.scout, schema: SCOUT_SCHEMA })))
  ...
  if (retryIdx.length) {
    log(`侦察疑似损坏（疑网络中途毁坏生成流）：${retryIdx.map((i) => A.files[i].label).join('、')}——各重试一次`)
    const retries = await parallel(retryIdx.map((i) => () =>
      agent(scoutPrompt(A.files[i], A), { label: `scout-retry:${A.files[i].label}`, model: M.scout, schema: SCOUT_SCHEMA })))
  ...
  log(`侦察完成 ${findings.filter(Boolean).length}/${A.files.length} 份${scoutSuspect.length ? `（${scoutSuspect.join('、')} 重试后仍疑损坏，校对表该份不可靠）` : ''}`)
  ...
  if (headingConflicts.length) log(`注意：${headingConflicts.join('、')} 源文件已带小标题但 headingPolicy=none——收尾时需问用户保留还是重做`)
  ...
  if (doVerify) {
    log(`核实：${vc.eligible} 项分 ${vc.chunks.length} 块并行送检${vc.excluded > 0 ? `，${vc.excluded} 项低优先级未送检（由精校按原文归一）` : ''}`)
    if (vc.overflow > 0) log(`核实：实体过多，${vc.overflow} 项超出 ${VERIFY_CHUNK * MAX_CHUNKS} 上限未送检`)
  }
  const dedupList = dedupListText(merged)
  const [vparts, dedupRes] = await parallel([
    () => vc.chunks.length
      ? parallel(vc.chunks.map((ct, i) => () => agent(verifyPrompt(ct, A), { label: `verify:${i + 1}/${vc.chunks.length}`, phase: 'Verify', model: M.verify, schema: VERIFY_SCHEMA })))
      : Promise.resolve([]),
    () => dedupList
      ? agent(dedupPrompt(dedupList, A), { label: 'dedup:semantic', phase: 'Verify', model: M.dedup, schema: DEDUP_SCHEMA })
  ...
  if (vc.chunks.length) {
    ...
    log(`核实完成：${verified.resolved.length} 项确认，${verified.unresolved.length} 项存疑（${goodParts.length}/${vc.chunks.length} 块返回）`)
    if (goodParts.length < vc.chunks.length) log(`核实：${vc.chunks.length - goodParts.length}/${vc.chunks.length} 块未返回（疑网络劣化），该批实体本轮未核实——网络稳定后可重跑`)
    netUnverified = pickNetworkUnverified(verified)
    if (netUnverified.length) log(`其中 ${netUnverified.length} 项因网络故障未核实——收尾时可向用户提供补核选项（networkUnverified）`)
  }
  dedup = dedupRes ? { suspects: cleanSuspects(dedupRes.suspects) } : null
  if (dedup && dedup.suspects.length) log(`疑似同指：标记 ${dedup.suspects.length} 组待人工确认`)
  glossary = renderGlossary(merged, verified, dedup, A)

  let positional = []
  if (scope.includes('refine')) {
    phase('Refine')
    positional = await pipeline(A.files,
      (f, _f, i) => findings[i] && agent(refinePrompt(f, glossary, findings[i], A),
        { label: `refine:${f.label}`, phase: 'Refine', model: M.refine, schema: REFINE_REPORT_SCHEMA }),
      (rep, f, i) => rep && agent(checkPrompt(f, findings[i].ending_anchor),
  ...
  if (failed.length) log(`未完成：${failed.join('、')}（主代理需按 SKILL.md Step 1–2 手动补做）`)
}
...
if (scope.includes('logic') && refinedPairs.length) {
  phase('Logic')
  const lreps = await parallel(refinedPairs.map(({ f }) => () =>
    agent(logicWritePrompt(f, A), { label: `logic:${f.label}`, model: M.logic, schema: LOGIC_REPORT_SCHEMA })))
  ...
  log(`逻辑顺序稿完成 ${logic.filter((l) => l.path).length}/${refinedPairs.length} 份${failedLogic.length ? `（${failedLogic.join('、')} 失败）` : ''}`)
  if (missLogic.length) log(`逻辑顺序稿疑漏小标题（按精校稿小标题覆盖核对，需抽查）：${missLogic.map((l) => `${l.label}:${l.missingSections.join('/')}`).join('；')}`)
}

phase('Deliver')
const [summary, timeline] = await parallel([
  () => (scope.includes('summary') && refined.length
    ? agent(summaryPrompt(A, refined), { label: 'summary', model: M.summary })
    : Promise.resolve(null)),
  () => (scope.includes('timeline') && refined.length
    ? agent(timelinePrompt(A, glossary, refined), { label: 'timeline', model: M.timeline })
    : Promise.resolve(null)),
])
