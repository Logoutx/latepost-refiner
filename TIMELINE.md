...
## July 2026 — The running reference table stops quietly losing notes

The tool carries a company's reference table — how every name, company, and term should be written — forward from one interview to the next, so a decision made once stays made. Several ways it could quietly lose an entry had built up. When the same name came back with a different note, the table kept the old note and dropped the new one. When two tables were joined and a section heading appeared more than once, only the first block was read. A row that had lost its heading was skipped entirely. And any single row could hold at most two notes. Each of these threw information away without a trace — and because this table is the memory a person is meant to review and correct, a silent loss is the worst kind. Now a conflicting note is kept beside the original; every repeated section is read; a stray row with no heading is kept and marked for re-filing rather than dropped; and when a row gathers more notes than it can carry, it keeps the earliest few and adds a visible marker so the overflow is plain to see. Nothing is discarded quietly — every case is surfaced for a person to settle.

## July 2026 — Splitting long transcripts so a cheaper model stays faithful

Cheaper models are fine on a normal-length interview but start quietly summarizing once a transcript runs long — folding away real detail to fit. Measured runs pinned where one such model's faithful range ends: it kept the full content on a mid-length transcript but dropped roughly a page on a much longer, four-speaker one. So each model can now declare the length past which it stops being faithful, and when a transcript exceeds it, the tool automatically splits the file into a few balanced parts along speaker-turn boundaries — each part comfortably inside the faithful range — refines them in parallel, and stitches them back together, so no detail is lost. The parts are kept as large as safely possible (an earlier over-eager split had fragmented sub-headings). Only models with a declared limit are affected; the default model, already faithful on long files, is untouched. Every automatic split is recorded in the run report in plain language, and a single switch turns all splitting off for anyone who wants the old one-pass behavior.

## July 2026 — Converging to three editions

The tool had grown a wide surface to maintain: five different ways to run it, and the API edition alone could be pointed at five different AI providers, mixed and matched per pipeline stage, with a mode that automatically reran a failed file on a pricier provider and a half-price overnight batch mode for large archives. Every one of those choices was one more setting that could drift out of sync or quietly break. The project settled on three editions instead: the two subscription-based editions each stay on their own platform — one running on a Claude subscription, one on a ChatGPT subscription — and the API-key edition narrows to a single AI provider, DeepSeek, paired with one search service, Tavily, for looking things up online. The provider-switching matrix, the per-stage model picker, the automatic cheap-then-premium rerun, and the half-price batch mode are all gone. What is left is a thinner surface to configure, and fewer places for the editions to drift apart.
