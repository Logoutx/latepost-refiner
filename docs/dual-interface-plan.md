...
**Research findings:**
- **Claude no-key already works.** The Claude Code skill's `scripts/claude-native.js` runs inside the Workflow tool; its `agent/parallel/pipeline/phase/log` are the Workflow globals (`build/bootstrap-cc.js`), billed to the Claude Code session — no `ANTHROPIC_API_KEY`. The key-requiring path (`engines/api.js`) is a separate edition.
